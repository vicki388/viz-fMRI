...
describe('Element', () => {
  ...
  it('should special-case text', () => {
    let el = new Element('div');
    el.set('text', 'woot');
    assert(0 === size(el.props));
    assert(1 == size(el.children));
  ...
  it('should special-case style', () => {
    ...
    assert('<div style="color:red"></div>' == el.toHTML());
    el.set('style', { color: 'red', background: 'blue' });
    assert('<div style="color:red;background:blue"></div>' == el.toHTML());
  });

  it('should admit string style', () => {
    let el = new Element('div');
    el.set('style', 'color:red');
    assert('<div style="color:red"></div>' == el.toHTML());
  });
